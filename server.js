...
// Middleware to verify admin token and active status
const verifyAdmin = async (req, res, next) => {
    try {
        ...
        if (adminData.isActive === false) {
            return res.status(403).json({ error: 'This admin account has been blocked' });
        }
...
app.post('/api/admin/block-user', verifyAdmin, async (req, res) => {
    try {
        ...
        const actorEmail = req.admin?.email || adminUid || 'system';
        ...
        console.log(`🚫 User ${targetUid} blocked by admin ${actorEmail}`);
        res.status(200).json({
            success: true,
...
// --- ADMIN: UNBLOCK USER ---
app.post('/api/admin/unblock-user', verifyAdmin, async (req, res) => {
    try {
        ...
        console.log(`🔓 User ${targetUid} unblocked by admin ${actorEmail}`);
        res.status(200).json({
            success: true,
...
app.post('/api/admin/login', async (req, res) => {
    try {
        ...
        // Check if this is an admin user (stored in a separate admins collection)
        ...
        res.json({
            ...
            admin: {
                ...
                status: adminData.status || 'employee',
...
// List all administrators (owner-only)
app.get('/api/admin/admins', verifyAdmin, async (req, res) => {
    try {
        if (req.admin.role !== 'super_admin' && req.admin.status !== 'owner') {
            return res.status(403).json({ error: 'Only the primary owner can manage administrators' });
        }
...
// Block an administrator (owner-only)
app.post('/api/admin/block-admin', verifyAdmin, async (req, res) => {
    try {
        const { targetEmail, blockReason } = req.body;

        if (req.admin.role !== 'super_admin' && req.admin.status !== 'owner') {
            return res.status(403).json({ error: 'Only the primary owner can block administrators' });
        }

        if (!targetEmail) {
            return res.status(400).json({ error: 'Target admin email is required' });
        }

        if (targetEmail === req.admin.email) {
            return res.status(400).json({ error: 'The primary owner cannot block their own account' });
        }
        ...
        if (targetData.role === 'super_admin') {
            return res.status(400).json({ error: 'The primary owner cannot be blocked' });
        }
        ...
        await targetDoc.ref.update({
            ...
            blockedAt: new Date().toISOString(),
            blockReason: blockReason || 'Blocked by primary owner'
        });

        await db.collection('adminActivity').add({
            adminEmail: req.admin.email,
            action: 'block_admin',
            details: `Blocked administrator ${targetEmail}`,
            timestamp: admin.firestore.FieldValue.serverTimestamp()
        });

        res.json({
            success: true,
            message: `Administrator ${targetEmail} blocked successfully`
        });
    } catch (error) {
        console.error('Block admin error:', error);
        res.status(500).json({ error: 'Failed to block administrator' });
    }
});

// Unblock an administrator (owner-only)
app.post('/api/admin/unblock-admin', verifyAdmin, async (req, res) => {
    try {
        const { targetEmail } = req.body;

        if (req.admin.role !== 'super_admin' && req.admin.status !== 'owner') {
            return res.status(403).json({ error: 'Only the primary owner can unblock administrators' });
        }
        ...
        await db.collection('adminActivity').add({
            ...
            details: `Unblocked administrator ${targetEmail}`,
            timestamp: admin.firestore.FieldValue.serverTimestamp()
        });

        res.json({
            success: true,
            message: `Administrator ${targetEmail} unblocked successfully`
        });
    } catch (error) {
        console.error('Unblock admin error:', error);
        res.status(500).json({ error: 'Failed to unblock administrator' });
    }
});
...
// Get single user
app.get('/api/admin/user/:uid', verifyAdmin, async (req, res) => {
    try {
        const { uid } = req.params;
        const userDoc = await db.collection('users').doc(uid).get();
        if (!userDoc.exists) {
...
app.get('/api/admin/withdrawals', verifyAdmin, async (req, res) => {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            withdrawals.push({ id: doc.id, ...data, timestamp });
        });
        res.json(withdrawals);
    } catch (error) {
        res.status(500).json({ error: error.message });
...
// Get all recharges
app.get('/api/admin/recharges', verifyAdmin, async (req, res) => {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            recharges.push({ id: doc.id, ...data, timestamp });
        });
        res.json(recharges);
    } catch (error) {
        res.status(500).json({ error: error.message });
...
// Approve withdrawal
app.post('/api/admin/withdrawal/approve/:id', verifyAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await db.runTransaction(async (t) => {
            ...
            if (withdrawData.status !== 'pending') {
                throw 'Withdrawal already processed';
            }
        ...
        // Log activity
        await db.collection('adminActivity').add({
            adminEmail: req.headers.authorization,
            action: 'approve_withdrawal',
            details: `Approved withdrawal ${id}`,
            timestamp: admin.firestore.FieldValue.serverTimestamp()
        });
        
        res.json({ success: true, message: 'Withdrawal approved' });
    } catch (error) {
        res.status(400).json({ error: typeof error === 'string' ? error : error.message });
    }
});

// Reject withdrawal (refund)
app.post('/api/admin/withdrawal/reject/:id', verifyAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { adminUid } = req.body;
        ...
        await db.runTransaction(async (t) => {
            ...
            t.update(userRef, {
                balance: admin.firestore.FieldValue.increment(withdrawData.amount)
            });
        ...
        await db.collection('adminActivity').add({
            ...
            details: `Rejected withdrawal ${id} and refunded`,
            timestamp: admin.firestore.FieldValue.serverTimestamp()
        });
    ...
    try {
        ...
        await db.runTransaction(async (t) => {
            ...
            if (rechargeData.status !== 'pending') {
                throw 'Recharge already processed';
            }
            
            // Add balance to user
            const userRef = db.collection('users').doc(rechargeData.uid);
            t.update(userRef, {
        ...
        await db.collection('adminActivity').add({
            ...
            details: `Approved recharge ${id} with ref ${adminVerifiedRef}`,
            timestamp: admin.firestore.FieldValue.serverTimestamp()
        });
...
// Give reward to user
app.post('/api/admin/give-reward', verifyAdmin, async (req, res) => {
    try {
        const { userIdentifier, amount, reason, adminUid } = req.body;
        
        if (!userIdentifier || !amount || !reason) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        
        // Find user by phone or UID
        let userQuery;
        if (userIdentifier.includes('@')) {
            userQuery = await db.collection('users').where('email', '==', userIdentifier).limit(1).get();
        } else if (userIdentifier.length > 20) {
            userQuery = await db.collection('users').where('uid', '==', userIdentifier).limit(1).get();
        } else {
            userQuery = await db.collection('users').where('phoneNumber', '==', userIdentifier.startsWith('+') ? userIdentifier : `+251${userIdentifier}`).limit(1).get();
        }
        ...
        await db.collection('adminActivity').add({
            adminEmail: req.headers.authorization,
            action: 'give_reward',
            details: `Gave ${amount} ETB reward to ${userIdentifier} for ${reason}`,
            timestamp: admin.firestore.FieldValue.serverTimestamp()
        });
        
        res.json({ success: true, message: 'Reward given successfully' });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
app.get('/api/admin/activity-log', verifyAdmin, async (req, res) => {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            activities.push({ id: doc.id, ...data, timestamp });
        });
        res.json(activities);
    } catch (error) {
        res.status(500).json({ error: error.message });
...
app.get('/api/admin/registration-status', verifyAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/set-registration-status', verifyAdmin, async (req, res) => {
    try {
        ...
        await db.collection('adminActivity').add({
            ...
            details: `Registration ${isOpen ? 'opened' : 'closed'}`,
            timestamp: admin.firestore.FieldValue.serverTimestamp()
        });
...
// Check registration status (for user registration)
...
// Update recharge request to include transactionRef
// (Modify existing recharge request route to accept transactionRef)
// Note: The existing route already accepts it, we just need to ensure it's stored
